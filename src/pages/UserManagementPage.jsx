...
import {
  ...
  XCircle, Lock, Loader2, RefreshCw, ArrowUp,
} from 'lucide-react';
...
export default function UserManagementPage() {
  ...
  async function handleConfirm() {
    ...
    try {
      ...
      if (action === 'promote-superadmin') updateData = { user_type: 'SUPERADMIN' };
  ...
  const filtered = users.filter(u => {
    ...
    const matchSearch  = !search || u.username?.toLowerCase().includes(q) || u.userid?.toLowerCase().includes(q);
  ...
  const CONFIRM_CONFIG = {
    'activate':   { title:'Activate User',    confirmLabel:'Activate',         confirmClass:'btn-primary' },
    'deactivate': { title:'Deactivate User',  confirmLabel:'Deactivate',       confirmClass:'btn-danger'  },
    'promote-admin':      { title:'Promote to Admin',      confirmLabel:'Promote to Admin',      confirmClass:'btn-primary' },
    'promote-superadmin': { title:'Promote to Super Admin',confirmLabel:'Promote to Super Admin',confirmClass:'btn-warning' },
    'demote-user':        { title:'Demote to User',        confirmLabel:'Demote to User',        confirmClass:'btn-danger'  },
  };
  ...
  return (
    <div className="fade-in">
      ...
      <div className="um-summary">
        <div className="um-sum-card"><p className="um-sv">{loading ? '…' : total}</p><p className="um-sl">Total Users</p></div>
        <div className="um-sum-card ok"><p className="um-sv">{loading ? '…' : active}</p><p className="um-sl">Active</p></div>
        <div className="um-sum-card warn"><p className="um-sv">{loading ? '…' : inactive}</p><p className="um-sl">Inactive</p></div>
        <div className="um-sum-card super"><p className="um-sv">{loading ? '…' : supers}</p><p className="um-sl">Super Admins</p></div>
      </div>
      ...
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">{loading ? 'Loading…' : `${filtered.length} of ${total} users`}</p>
        </div>
        <div className="page-actions">
          ...
          <button className="btn btn-secondary" onClick={() => fetchUsers(true)} disabled={refreshing}>
            ...
            <tbody>
              {filtered.map(u => {
                ...
                return (
                  ...
                  >
                    {tooltip === u.userid && (
                      <div className="um-tooltip">🔒 SUPERADMIN accounts cannot be modified</div>
                    )}
                        ...
                        <div style={{ display:'flex', gap:6, flexWrap:'wrap' }}>
                          ...
                          {actable && u.user_type === 'USER' && (
                            <button className="btn btn-secondary btn-sm"
                              onClick={() => setConfirm({ action:'promote-admin', user:u })}
                              title="Promote to Admin">
                              <ArrowUp size={13} /> Make Admin
                            </button>
                          )}
                          {actable && isSuperAdmin && u.user_type === 'USER' && (
                            <button className="btn btn-warning btn-sm"
                              onClick={() => setConfirm({ action:'promote-superadmin', user:u })}
                              title="Promote to Super Admin">
                              <Crown size={13} /> Make Super Admin
                            </button>
                          )}
                          {actable && isSuperAdmin && u.user_type === 'ADMIN' && (
                            <>
                              <button className="btn btn-warning btn-sm"
                                onClick={() => setConfirm({ action:'promote-superadmin', user:u })}
                                title="Promote to Super Admin">
                              ...
                              <button className="btn btn-secondary btn-sm"
                                onClick={() => setConfirm({ action:'demote-user', user:u })}
                                title="Demote to User">
                                <User size={13} /> Make User
                              </button>
                            </>
                          )}
                          {actable && isAdmin && !isSuperAdmin && u.user_type === 'ADMIN' && (
                            <button className="btn btn-secondary btn-sm"
                              onClick={() => setConfirm({ action:'demote-user', user:u })}
                              title="Demote to User">
                              <User size={13} /> Make User
      ...
      {confirm && (
        <ConfirmModal
          title={actionLoading ? 'Processing…' : (CONFIRM_CONFIG[confirm.action]?.title || 'Confirm')}
          message={confirmMsg(confirm.action, confirm.user.username)}
          confirmLabel={actionLoading ? 'Processing…' : (CONFIRM_CONFIG[confirm.action]?.confirmLabel || 'Confirm')}
          confirmClass={CONFIRM_CONFIG[confirm.action]?.confirmClass || 'btn-primary'}
